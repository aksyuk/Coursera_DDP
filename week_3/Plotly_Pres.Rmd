...
pl1 <- plot_ly(dt.2, x = ~Year, y = ~Trade.Value.000.USD.others, 
    ...
    layout(yaxis = list(title = 'US Dollars, thousands'), barmode = 'stack',
           legend = list(x = 0, y = 0.95))

pl2 <- dt[European_Union == 1, ] %>% 
    plot_ly(y = ~Trade.Value.USD/10^3, name = 'EU', 
            showlegend = FALSE, x = ~Year, 
            type = 'box', line = list(color = 'purple'),
            height = 400) %>% 
    add_trace(data = dt[European_Union != 1, ], y = ~Trade.Value.USD/10^3, 
              name = 'Others', x = ~Year, showlegend = FALSE,
              line = list(color = 'green')) %>% 
    layout(boxmode = "group", yaxis = list(title = 'US Dollars, thousands'))

subplot(pl1, pl2) %>% config(displayModeBar = F) 

```

<font size = 5><em>Import of butter to Russian Federation from Europe stopped after 2014 due to trade sanctions.</em></font>
